    ...
    return this.schedule(async () => {
      ...
      await transaction(this.db, async () => {
        for (const part of chunk(ids, this.db.maxBoundParams)) {
          ...
          // non-external-content table like these. Keep this list in sync with
          // `src/worker/argus-demo-store.ts`'s `SESSION_SCOPED_TABLES` (a full unconditional wipe for
          // the demo's nightly reseed, rather than this method's id-scoped delete) if a new
          // session-scoped table is ever added.
          await run(
            this.db,
