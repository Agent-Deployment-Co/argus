...
// no filesystem cwd, so a conversation started in a claude.ai Project resolves to that project's name
// (via the projects auxiliary, project_uuid → name); conversations outside a project fall back to a
// fixed "claude.ai chat" label. It does not canonicalize subagents (there are none), dedupe by
// provider message id (the file-level AlternateRepresentation handles duplicates), or observe friction
// (the cache exposes no interrupt/permission/compaction markers).
export const claudeChatProducer: NativeProducer = {
  ...
  capabilities: {
    ...
    unknownProjectLabel: () => "claude.ai chat",
  },
