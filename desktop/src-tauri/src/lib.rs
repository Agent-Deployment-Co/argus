...
//
// On launch it starts `argus run` (index + serve, plus a Hub-only sync leg that stays dormant until
// a Hub URL + key are configured, all supervised in one process) on a free local port and exposes a
// tray menu whose items map onto the CLI's command surface. Logs are written to rotating files.
// "Open Argus" opens the user's default browser at the served URL; the only embedded webview is the
...
fn spawn_sidecar(app: &AppHandle) -> Result<CommandChild, String> {
    ...
    let args = vec![
        ...
        "--port".to_string(),
        port.to_string(),
        "--hub-only-sync".to_string(),
    ];
    log::info!("starting argus on port {port} (Hub sync when configured)");
    let mut command = app
        .shell()
        .sidecar("argus")
