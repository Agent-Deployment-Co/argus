...
[agents](/glossary#agent), indexes it into a local database and shows you the
...
Argus checks for new [sessions](/glossary#session) periodically. When you finish a
conversation with an agent, or an existing one grows, Argus notices the change on
its next pass and [indexes](/glossary#index) it, reading the session once and
...
your machine unless you choose to [sync](/glossary#sync) a usage snapshot to an
[Argus Hub](/glossary#argus-hub) run by your company.
